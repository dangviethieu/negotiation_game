...
class C(BaseConstants):
    NAME_IN_URL = 'negotiation_vn_game_info'
    PLAYERS_PER_GROUP = 2
    NUM_ROUNDS = 1
    ENDOWMENT = cu(100)
    BUYER_ROLE = 'Bên mua'
...
class Player(BasePlayer):
    ...
    seller_offer_new_bribe = models.IntegerField(initial=0)
    fixed_sum_proposed = models.IntegerField(initial=0, min=0, max=C.ENDOWMENT, label="Giá trị đề nghị")
    accepted_negotiation_from_fixed_sum = models.IntegerField(
        widget=widgets.RadioSelect,
        choices=[
            [1, 'Chấp nhận'],
            [2, 'Từ chối'],
            [3, 'Từ chối với đề xuất mới']
        ]
    )
    percentage_proposed = models.IntegerField(initial=2, choices=[2, 5, 10], label="Mức % đề nghị")
...
def group_by_arrival_time_method(subsession, waiting_players):
    ...
    if len(players) > 1:
        ...
        while player_count < len(players):
            if players[player_count].address != players[0].address:
                return [players[0], players[player_count]]
            player_count += 1
